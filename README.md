...
В режиме «Микрофон» возможно тихое попадание системного звука. Это принятый
компромисс ради качества голоса.
...
`main.py` и `main.spec` являются устаревшими файлами и не используются
актуальной командой запуска или сборки.

## Скриншоты

Перед публикацией добавьте в этот раздел обезличенные снимки начального окна,
выбранной области и состояния записи. На изображениях не должно быть рабочих
данных, личных путей или содержимого других приложений.

## Лицензии

Лицензия собственного кода пока не выбрана владельцем проекта. До выбора
лицензии стандартные права на исходный код сохраняются за правообладателем.

Приложение использует сторонние библиотеки и включает FFmpeg. Сведения о
компонентах приведены в [THIRD_PARTY_NOTICES.txt](THIRD_PARTY_NOTICES.txt), а
текст лицензии поставляемой сборки FFmpeg — в
[FFMPEG-LICENSE.txt](FFMPEG-LICENSE.txt).
